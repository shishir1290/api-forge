...
import type {
  RequestConfig, ResponseData, Collection,
  CollectionFolder, Environment, HistoryItem, Tab,
  TeamMember, TeamWorkspace, TeamRole,
} from '@/types';
...
function addFolderToFolders(
  ...
  return folders.map((f) => {
    ...
    return { ...f, folders: addFolderToFolders(f.folders, parentFolderId, newFolder) };
  });
}
...
function updateFolderInFolders(
  ...
  return folders.map((f) => {
    ...
    return { ...f, folders: updateFolderInFolders(f.folders, folderId, updates) };
  });
}
...
function deleteFolderFromFolders(
  ...
  return folders
    ...
    .map((f) => ({ ...f, folders: deleteFolderFromFolders(f.folders, folderId) }));
}
...
interface AppState {
  ...
  updateRequestInCollection: (collectionId: string, requestId: string, updates: Partial<RequestConfig>) => void;

  // Folders — parentFolderId=null means root of collection
  addFolderToCollection: (collectionId: string, name: string, parentFolderId?: string | null) => void;
  ...
  addRequestToFolder: (collectionId: string, folderId: string, request: RequestConfig) => void;
  deleteRequestFromFolder: (collectionId: string, requestId: string) => void;
  updateRequestInFolder: (collectionId: string, requestId: string, updates: Partial<RequestConfig>) => void;
  ...
  updateWorkspace: (updates: Partial<Pick<TeamWorkspace, 'name' | 'description'>>) => void;
}
    ...
    (set, get) => ({
      ...
      addRequestToCollection: (collectionId, request) =>
        set((s) => ({
          collections: s.collections.map((c) =>
            c.id === collectionId
              ? { ...c, requests: [...c.requests, request], updatedAt: new Date().toISOString() }
              : c
          ),
        })),
          ...
          collections: s.collections.map((c) =>
            c.id === collectionId
              ? {
                  ...
                  folders: updateRequestInFolders(c.folders, requestId, updates),
                  updatedAt: new Date().toISOString(),
                }
              : c
          ),
        ...
        set((s) => ({
          collections: s.collections.map((c) => {
            ...
            if (!parentFolderId) {
              ...
              return { ...c, folders: [...c.folders, newFolder], updatedAt: new Date().toISOString() };
            }
            // Add as sub-folder
            return {
              ...c,
      ...
      openTab: (request) => {
        ...
        const tab: Tab = { id: uuidv4(), requestId: request.id, name: request.name, isDirty: false };
        set((s) => ({
          tabs: [...s.tabs, tab],
          activeTabId: tab.id,
      ...
      closeTab: (tabId) => {
        ...
        set((s) => {
          ...
          return { tabs: newTabs, activeTabId: newActiveId, requests: restRequests, responses: restResponses };
        });
      },
      ...
      updateRequest: (tabId, updates) =>
        set((s) => ({
          requests: { ...s.requests, [tabId]: { ...s.requests[tabId], ...updates } },
          tabs: s.tabs.map((t) =>
            t.id === tabId
              ? { ...t, isDirty: true, name: updates.name !== undefined ? updates.name : t.name }
              : t
          ),
        })),
      ...
      // ── Environments ──
      addEnvironment: (name) => {
        const env: Environment = { id: uuidv4(), name, variables: [], isActive: false };
      ...
      inviteMember: (name, email, role) => {
        const AVATAR_COLORS = ['#58a6ff','#bc8cff','#3fb950','#f0883e','#ff7b72','#ffa657','#79c0ff'];
        ...
        set((s) => ({
          workspace: { ...s.workspace, members: [...s.workspace.members, member] },
        }));
      },
      ...
      updateMemberRole: (memberId, role) =>
        set((s) => ({
          workspace: {
            ...s.workspace,
            members: s.workspace.members.map((m) => m.id === memberId ? { ...m, role } : m),
          },
        })),
